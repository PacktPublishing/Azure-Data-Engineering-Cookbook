...
if($createstorageaccount -eq $true)
{
	...
	# Create destination storage container
	New-AzStorageContainer -Name ordersummary -Context $storage.context
...
If($uploadfiles -eq $true)
{
	...
	foreach($file in $files){
		Set-AzStorageBlobContent -File $file.FullName -Context $storage.context -Blob data/$file -Container orders -Force
	}
}
